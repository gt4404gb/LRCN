...
#————————————————————————————数据集加载————————————————————————————————
x_train = pd.read_csv("../dataset/UNSW-NB15/kaggle_UNSW_NB15_training.csv", low_memory=False)
num_cols = x_train.shape[1] #获取列数
#提取出最后一列为y
y_train = x_train.pop(x_train.columns[-1]).values

x_test = pd.read_csv("../dataset/UNSW-NB15/kaggle_UNSW_NB15_testing.csv", low_memory=False)
...
model = CNN.CNNClassifier(input_size=x_train.shape[1], hidden_size=hidden_dim, output_size=class_number).to(dev)
...
for epoch in range(epochs):
    ...
    for batch_idx, (x_batch, y_batch) in enumerate(train_loader):
        ...
        loss = CNN.cnn_loss(y_batch, y_hat)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
